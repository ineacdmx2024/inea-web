...
function Planeacion() {
  ...
  const fetchFijos = async () => {
    try {
      const response = await fetch(
        `https://inea-web-backend-production.up.railway.app/api/servedus?filters[Fijo][$eq]=true&populate=*&sort[0]=Orden:asc&pagination[limit]=10`
      );
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://inea-web-backend-production.up.railway.app/api/servedus?filters[$or][0][Fijo][$eq]=false&filters[$or][1][Fijo][$null]=true&populate=*&sort[0]=Fecha:desc&pagination[limit]=${noticiasPorPagina}&pagination[start]=${start}`
      );
      const result = await response.json();
      ...
      if (!result.data || result.data.length === 0) {
        ...
        const responseAll = await fetch(
          `https://inea-web-backend-production.up.railway.app/api/servedus?populate=*&sort[0]=Fecha:desc&pagination[limit]=${noticiasPorPagina}&pagination[start]=${start}`
        );
                    ...
                    >
                      ...
                      <div className="flex justify-center my-4">
                        <Link
                          ...
                          className="bg-[#611432] text-white text-center py-2 px-4 hover:bg-white hover:text-[#611432] border-2 border-[#611432] rounded-full"
                        >
                          Continuar leyendo
                        </Link>
                      </div>
