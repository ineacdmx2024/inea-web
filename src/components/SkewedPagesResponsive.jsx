    ...
    return (<div>
        <div className="flex flex-wrap bg-white p-3 top-0 w-full">
            {alcaldias.map((elemento) => (
                <button
                    ...
                    type="button"
                    className="text-[#8A1B39] hover:text-white border border-[#8A1B39] hover:bg-[#7c1833] focus:ring-4 focus:outline-none focus:ring-blue-300 text-xs rounded-full px-3 py-2 text-center me-2 mb-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-500 dark:focus:ring-blue-800 text-lg flex"
        ...
        <div className="sticky container mx-auto flex-column h-full bg-white">
            {datos.map((pageData, index) => (
                <div
                    key={index}
                    className={`sticky mb-5  border border-slate-300 rounded-lg ${index === currentPage ? "visible" : "hidden"}`}
                >
                    {/* Lado izquierdo- Arriba */}
                    <div className="left-0 top-0">
                        <div className="flex-column items-center justify-center text-center p-2 h-full bg-cover bg-[#13322b] rounded-t-lg text-white">
                            <h1 className="mt-3 uppercase text-[25px] text-center">{pageData.title}</h1>
                            <div>
                                <p className="text-[18px] text-center">{pageData.nom}</p>
                                <p className="text-[18px] text-center">Teléfono: {pageData.tel}</p>
                                <p className="text-[18px] text-center">Email: {pageData.email}</p>
                            </div>
