          ...
          <div className="relative w-full overflow-hidden">
            <div
              ...
              onScroll={() => {
                ...
                const cardWidth =
                  scrollRef.current.children[1]?.offsetWidth || 1;
                const spacing = parseInt(
                  window.getComputedStyle(scrollRef.current).gap || 0
                );
                const index = Math.round(
                  scrollRef.current.scrollLeft / (cardWidth + spacing)
                );
                setActiveIndex(index);
              }}
              className={`${
            ...
            {isMobile && (
              <>
                ...
                <button
                  onClick={() =>
                    scrollToCard(Math.min(activeIndex + 1, items.length - 1))
                  }
                ...
                {items.map((_, idx) => (
                  <div
                    ...
                    className={`h-2 w-2 rounded-full ${
                      idx === activeIndex ? "bg-[#611232]" : "bg-gray-300"
                    }`}
                  ></div>
                ))}
              </div>
